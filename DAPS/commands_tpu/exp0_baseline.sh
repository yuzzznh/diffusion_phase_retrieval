#!/bin/bash
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# [실험 0] DAPS Baseline (LatentDAPS with Langevin Dynamic) - TPU 버전
...
# ============================================================
# [실험 0] Sanity Check - 1 image (TPU)
# ============================================================
if [ "$RUN_1" = true ]; then
    echo "========== [실험 0] 1 image sanity check (TPU) =========="
    ...
    save_dir=results/exp0_baseline_tpu/imagenet_1img \
    num_samples=4 \
    ...
    optimization_step=-1 \
    data.end_id=1 \
    name=exp0_sanity_check_tpu \
    use_tpu=true
fi

# ============================================================
# [실험 0] Main Experiment - 10 images (TPU)
# ============================================================
if [ "$RUN_10" = true ]; then
    echo "========== [실험 0] 10 images main experiment (TPU) =========="
    python posterior_sample.py \
    +data=test-imagenet \
    +model=imagenet256ldm \
    +task=phase_retrieval \
    +sampler=latent_edm_daps \
    task_group=ldm_langevin \
    save_dir=results/exp0_baseline_tpu/imagenet_10img \
    num_samples=4 \
    sampler.diffusion_scheduler_config.num_steps=2 \
    sampler.annealing_scheduler_config.num_steps=50 \
    repulsion_scale=0.0 \
...
# ============================================================
# [실험 0] Final Eval - 100 images (TPU)
# ============================================================
if [ "$RUN_100" = true ]; then
    echo "========== [실험 0] 100 images final eval (TPU) =========="
    python posterior_sample.py \
    +data=test-imagenet \
    +model=imagenet256ldm \
    +task=phase_retrieval \
    +sampler=latent_edm_daps \
    task_group=ldm_langevin \
    save_dir=results/exp0_baseline_tpu/imagenet_100img \
    num_samples=4 \
    sampler.diffusion_scheduler_config.num_steps=2 \
    sampler.annealing_scheduler_config.num_steps=50 \
    repulsion_scale=0.0 \
