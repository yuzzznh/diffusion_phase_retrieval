...
def sample_in_batch(sampler, model, x_start, operator, y, evaluator, verbose, record, batch_size, gt, args, root, run_id, resample=False):
    """
        posterior sampling in batch
    """
    samples = []
    trajs = []
    timing_list = []  # [Timing] Collect timing stats per batch
    for s in range(0, len(x_start), batch_size):
        ...
        cur_samples = sampler.sample(model, cur_x_start, operator, cur_y, evaluator, verbose=verbose, record=record, resample=resample, gt=cur_gt)

        samples.append(cur_samples)

        # [Timing] Collect timing stats if available
        if hasattr(sampler, 'timing_stats'):
            timing_list.append(sampler.timing_stats.copy())
        if record:
    ...
    return torch.cat(samples, dim=0), trajs, timing_list
...
def main(args):
    ...
    full_samples = []
    full_trajs = []
    all_timing_stats = []  # [Timing] Collect all timing stats
    for r in range(args.num_runs):
        print(f'Run: {r}')
        x_start = sampler.get_start(images.shape[0], model)
        samples, trajs, timing_list = sample_in_batch(sampler, model, x_start, operator, y, evaluator, verbose=True, record=args.save_traj,
                                         batch_size=args.batch_size, gt=images, args=args, root=root, run_id=r, resample=args.resample)
        full_samples.append(samples)
        full_trajs.append(trajs)
        all_timing_stats.extend(timing_list)  # [Timing] Aggregate
    full_samples = torch.stack(full_samples, dim=0)

    # evaluate and log metrics
    results = evaluator.report(images, y, full_samples)

    # [Timing] Add timing stats to results
    if all_timing_stats:
        avg_total_time = sum(t['total_time'] for t in all_timing_stats) / len(all_timing_stats)
        avg_opt_time = sum(t['opt_time'] for t in all_timing_stats) / len(all_timing_stats)
        avg_opt_ratio = sum(t['opt_ratio'] for t in all_timing_stats) / len(all_timing_stats)
        avg_diffusion_time = sum(t.get('diffusion_time', 0) for t in all_timing_stats) / len(all_timing_stats)
        avg_diffusion_ratio = sum(t.get('diffusion_ratio', 0) for t in all_timing_stats) / len(all_timing_stats)

        # [Memory] Aggregate peak memory stats across batches (max of all batches)
        max_diffusion_peak = max(t.get('diffusion_peak_memory_mb', 0) for t in all_timing_stats)
        max_optimization_peak = max(t.get('optimization_peak_memory_mb', 0) for t in all_timing_stats)

        results['timing'] = {
            'avg_total_time_sec': round(avg_total_time, 3),
            'avg_opt_time_sec': round(avg_opt_time, 3),
            'avg_opt_ratio_percent': round(avg_opt_ratio * 100, 2),
            'avg_diffusion_time_sec': round(avg_diffusion_time, 3),
            'avg_diffusion_ratio_percent': round(avg_diffusion_ratio * 100, 2),
            'num_batches': len(all_timing_stats)
        }

        # [Memory] Add memory stats to results
        results['memory'] = {
            'diffusion_peak_memory_mb': round(max_diffusion_peak, 2),
            'optimization_peak_memory_mb': round(max_optimization_peak, 2),
        }

        # [Tracking] Add per-step tracking stats if available
        if 'opt_iters_per_step' in all_timing_stats[0]:
            # Concatenate per-sample tracking across batches
            # Structure: [sample_idx][step] for each batch -> flatten to [global_sample_idx][step]
            def concat_per_sample(key):
                """Concatenate per-sample data across all batches."""
                all_samples = []
                for t in all_timing_stats:
                    if key in t and t[key]:
                        all_samples.extend(t[key])  # Each t[key] is [sample_idx][step]
                return all_samples

            def concat_per_batch(key):
                """Concatenate per-batch data across all batches."""
                all_steps = []
                for t in all_timing_stats:
                    if key in t and t[key]:
                        all_steps.append(t[key])  # Each t[key] is [step]
                return all_steps

            results['timing']['total_opt_iters'] = sum(t['total_opt_iters'] for t in all_timing_stats)
            results['timing']['avg_opt_iters_per_batch'] = round(
                sum(t['total_opt_iters'] for t in all_timing_stats) / len(all_timing_stats), 1
            )
            results['tracking'] = {
                # All per-sample stats: [sample_idx][step]
                'opt_iters_per_step': concat_per_sample('opt_iters_per_step'),
                'opt_time_per_step': concat_per_sample('opt_time_per_step'),  # per-sample opt time
                'error_before_opt': concat_per_sample('error_before_opt'),
                'error_after_opt': concat_per_sample('error_after_opt'),
                'dist_to_gt': concat_per_sample('dist_to_gt'),
                'drift_during_mcmc': concat_per_sample('drift_during_mcmc'),
                'latent_norm': concat_per_sample('latent_norm'),
                # Per-batch stats: [batch_idx][step]
                'diffusion_time_per_step': concat_per_batch('diffusion_time_per_step'),
            }
