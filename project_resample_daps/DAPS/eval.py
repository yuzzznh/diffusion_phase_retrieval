...
class Evaluator:
    ...
    def display(self, result_dicts):
        table = Table('results')
        average, std = {}, {}
        eval_keys = [k for k in result_dicts.keys() if k in __EVAL_FN_CMP__]
        ...
        # for average
        table.add_row(['' for _ in eval_keys])
        table.add_row(['mean' for _ in eval_keys])
        table.add_row(average.values())
        table.add_row(['' for _ in eval_keys])
        table.add_row(['std' for _ in eval_keys])
    ...
    def log_wandb(self, result_dicts, batch_size):
        ...
        for s in range(batch_size):
            log_dict = {key: result_dicts[key][get_eval_fn_cmp(key)][s] for key in eval_keys}
            wandb.log(log_dict)
        log_dict = {key: np.mean(result_dicts[key][get_eval_fn_cmp(key)]) for key in eval_keys}
